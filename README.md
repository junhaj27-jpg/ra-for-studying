...
AIDLC-RA를 사용할 때는 다음 사항을 반드시 전제로 해야 합니다.

- 생성 결과는 초안입니다. 실제 인허가 제출 문서, 품질문서 승인본, 규제 판단 결과로 바로 사용하면 안 됩니다.
- 의료 진단, 치료 결정, 처방 추천, 응급상황 판단, 임상적 유효성 판단에 사용하면 안 됩니다.
- 최신 법령, 고시, 가이드라인, 국제표준, 기관별 양식, 품질시스템 절차와 반드시 대조해야 합니다.
- ISO 14971 위험도 계산과 IEC 62304 추적성 결과는 포트폴리오용 보조 로직이며, 제조사의 공식 위험허용 기준을 대체하지 않습니다.
- RAG 근거 문장과 출처는 사용자가 원문과 일치 여부를 확인해야 합니다. 잘못된 업로드 자료나 오래된 기준을 근거로 삼을 수 있습니다.
- 환자정보, 개인정보, 영업비밀, 승인되지 않은 임상자료, 외부 반출이 제한된 회사 문서를 샘플 데이터로 넣지 않아야 합니다.
- 문서 간 불일치 검출 결과는 누락 가능성이 있으므로 RA 담당자, QA, 개발자, 시험 담당자의 교차 검토가 필요합니다.
- Word/PDF 출력물은 포트폴리오 시연용 템플릿 결과이며, 실제 제출 전 문서번호, 승인권자, 개정이력, 서명, 품질절차 반영이 필요합니다.
- 이 프로젝트는 RA 실무 흐름 이해를 보여주기 위한 포트폴리오이며, 법적 효력이 있는 규제 자문 도구가 아닙니다.
